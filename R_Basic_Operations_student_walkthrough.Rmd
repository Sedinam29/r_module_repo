...
```{r}
#use tghe right tools for the things it can
a <- 10
class(a)
```
...
```{r}
substr('How much wood could a woodchuck chuck?', start <- 5, stop <- 14)
```
...
```{r}
# USEFUL FOR CLEANING DATA.. replacing values 
...
1.  Create two strings with your first and last name, respectively. Then concatenate them, and print the new value as "My name is \<your newly created concatenated variable will go here\>"
    ...
    ```{r}
    first_string <- 'Hey There!'
    second_string <- 'Well module 5!!'
    fs_string <- 'Go big, or go home'
    con_string <- paste( 'My 3 strings all together:',first_string, second_string, fs_string)
...
print(paste("The value of a boolean is","%s", "or", bool_T, bool_F))
```
